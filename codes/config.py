...
class Config():
    def __init__(self, path):
        ...
        with open(self.path,'r',encoding='utf-8') as f:
            ...
            #for line in f:
            #    data += line.split('//')[0]+'\n'
            #self._conf.update(json.loads(data,object_hook=OrderedDict))
    ...
    def load(self,is_train=True):
        ...
        if is_train:
            ...
            if 'debug' in self._conf['name']:
                self._conf['train']['val_freq'] = 8
                self._conf['logger']['print_freq'] = 2
                self._conf['logger']['save_checkpoint_freq'] = 8
                self._conf['train']['lr_decay_iter'] = 10
        else:  # test
    ...
    def checkResume(self):
        if self._conf['path']['resume_state']:
            ...
            logger.info('Set [pretrain_model_G] to ' + self._conf['path']['pretrain_model_G'])
            if 'gan' in self._conf['model']:
                self._conf['path']['pretrain_model_D'] = osp.join(self._conf['path']['models'],
                                                        '{}_D.pth'.format(state_idx))
                logger.info('Set [pretrain_model_D] to ' + self._conf['path']['pretrain_model_D'])
        return self
