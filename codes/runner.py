...
class Runner:
    def __init__(self, path,tf_log_dir='../tf_logger/'):
        ...
        util.setup_logger(None, self.config['path']['log'], 'train', level=logging.INFO, screen=True)
        util.setup_logger('val', self.config['path']['log'], 'val', level=logging.INFO)
    ...
    def model(self):
        if not self._model:
            raise NotImplementedError('invoke prepare before utilizing self.model')
        return self._model
...
class TrainRunner(Runner):
    def __init__(self, path='options/train/train_ESRGAN.yaml'):
        super().__init__(path)

    def prepare(self):
        # * 生成tensorboard logger的代码转为lazy property
